...
*/
    public ResultadoCarrera(Element result){
       // TO-DO: donar valor als atributs de la classe a partir d'un element "Result" extret del XML.
        Element drv= (Element) result.getElementsByTagName("driver").item(0);
        this.d = new Driver(drv);
        constructor = result.getElementsByTagName("Constructor").item(0).getFirstChild().getNodeValue();
        initialPos = Integer.parseInt(result.getElementsByTagName("Grid").item(0).getFirstChild().getNodeValue());
        finalPos =  Integer.parseInt(result.getElementsByTagName("position").item(0).getFirstChild().getNodeValue());
        timeMillis = Long.parseLong(result.getElementsByTagName("time").item(0).getFirstChild().getNodeValue());
        completedLaps =  Integer.parseInt(result.getElementsByTagName("Laps").item(0).getFirstChild().getNodeValue());
        rankFastesLap = Integer.parseInt(result.getElementsByTagName("FastestLap").item(0).getFirstChild().getNodeValue());
        finisher = Boolean.parseBoolean(result.getElementsByTagName("Status").item(0).getFirstChild().getNodeValue());
    }
